  ...
  <div class="container" id="container">
    ...
    <div class="row" style="z-index: 2;">
      <div class="row col-md-12 col-sm-5 col-5">
        <div class="col-md-3 col-sm-12 col-12 justify-content-center" style="display:flex;">
          <!-- 是否依照距離排列勾選框 -->
          <b-form-checkbox
          ...
          <!-- 升冪/降冪 -->
          <b-form-checkbox v-model="sortRadio.find(each => each.type==='distance').sortCheck" name="check-button" switch @change="setSortByDescOrAsc('distance')"
            :disabled="!sortRadio.find(each => each.type==='distance').check">
            <b>{{sortRadio.find(each => each.type==='distance').sortLabel }}</b>
          </b-form-checkbox>
        </div>
        <div class="col-md-3 col-sm-12 col-12 justify-content-center" style="display:flex;">
          <!-- 是否依照價錢排列勾選框 -->
          ...
          ><b>{{sortRadio.find(each => each.type==='price_level').label }}</b>
          </b-form-checkbox>
          <!-- 升冪/降冪 -->
          <b-form-checkbox v-model="sortRadio.find(each => each.type==='price_level').sortCheck" name="check-button" switch @change="setSortByDescOrAsc('price_level')"
            :disabled="!sortRadio.find(each => each.type==='price_level').check">
            <b>{{sortRadio.find(each => each.type==='price_level').sortLabel }}</b>
          </b-form-checkbox>
        </div>
        <div class="col-md-3 col-sm-12 col-12 justify-content-center" style="display:flex;">
          <!-- 是否依照評分排列勾選框 -->
          ...
          <!-- 升冪/降冪 -->
          <b-form-checkbox v-model="sortRadio.find(each => each.type==='rating').sortCheck" name="check-button" switch @change="setSortByDescOrAsc('rating')"
            :disabled="!sortRadio.find(each => each.type==='rating').check" >
            <b>{{sortRadio.find(each => each.type==='rating').sortLabel }}</b>
          </b-form-checkbox>
        </div>
        <div class="col-md-3 col-sm-12 col-12 justify-content-center" style="display:flex;">
          <!-- 是否依照名稱排列勾選框 -->
          ...
          <!-- 升冪/降冪 -->
          <b-form-checkbox v-model="sortRadio.find(each => each.type==='name').sortCheck" name="check-button" switch @change="setSortByDescOrAsc('name')"
            :disabled="!sortRadio.find(each => each.type==='name').check">
            <b>{{sortRadio.find(each => each.type==='name').sortLabel }}</b>
          </b-form-checkbox>
        </div>
      </div>
      <div class="row col-md-2 col-sm-7 col-7 order-1 order-sm-1 order-md-2" :style="{height:listHeight, overflowY:'auto', paddingRight: '0', margin:'0'}">
          <div class="col-md-12 col-sm-6 col-6 place_item" :key="m.place_id" v-for="(m) in places">
            <button type="button" class="btn btn-link" @click="onClickItem(m.place_id)">{{m.name}}</button>
          </div>
      </div>
      <div class="row col-md-10 col-sm-12 col-12 order-2 order-sm-2 order-md-1" id="map_container">
...
export default {
  ...
  data () {
    return {
      sortRadio: [
        { type: 'distance', check: false, order: 3, label: '距離', sortLabel: '由遠到近', sortCheck: false },
        { type: 'price_level', check: false, order: 3, label: '價錢', sortLabel: '由高到低', sortCheck: false },
        { type: 'rating', check: false, order: 3, label: '評分', sortLabel: '由低到高', sortCheck: true },
        { type: 'name', check: false, order: 3, label: '名稱', sortLabel: '筆畫多到少', sortCheck: false }
      ],
      mapWidth: '',
      listHeight: '600px',
  ...
  mounted () {
    // 取得外框寬度給予google map
    this.mapWidth = (document.getElementById('map_container').clientWidth - 15) + 'px'
    // 視窗大小改變時 改變餐廳列表之高度
    window.onresize = () => {
      this.mapWidth = (document.getElementById('map_container').clientWidth - 15) + 'px'
  ...
  methods: {
    ...
    compareFunction (array, sort) {
      ...
      const returnAry = [].concat(numAry).concat(eAry).concat(cAry)
      if (sort === 'asc') {
        return returnAry
      } else {
    ...
    setSortByType (type) {
      this.placeIdInList = ''
      this.sortRadio.forEach(each => { each.check = false })
      this.sortRadio.find(each => each.type === type).check = !this.sortRadio.find(each => each.type === type).check
      this.sortPlaceList(type)
    },
    // 改變升冪降冪排序方式
    setSortByDescOrAsc (type) {
      this.placeIdInList = ''
      this.sortRadio.find(each => each.type === type).sortCheck = !this.sortRadio.find(each => each.type === type).sortCheck
      this.sortPlaceList(type)
    },
    sortPlaceList (type) {
      // 依照勾選順序進行排序
